...
# Squid Proxy One-Key Installation Script (No Authentication Version)
# Author: Auto-generated
# Description: Automatically download, install, configure and start Squid proxy server
#              This version does NOT require proxy username/password (open proxy)
...
# Configure Squid (No Authentication)
configure_squid() {
    print_status "Configuring Squid with custom settings (No Auth mode)..."
...
EOF
    ...
    print_success "Squid configuration updated (No Authentication)"
}
...
show_info() {
    ...
    echo
    print_warning "NO AUTHENTICATION - This is an OPEN proxy!"
    print_warning "Anyone who knows the IP and port can use this proxy."
    print_warning "Make sure to restrict access via firewall if needed."
    echo
    echo "Usage Examples:"
    echo "  - HTTP Proxy: http://$SERVER_IP:5566"
    echo "  - Test with curl:"
    echo "    curl -x http://$SERVER_IP:5566 http://httpbin.org/ip"
    echo
    ...
    echo
    print_warning "This proxy has NO authentication. Consider using the authenticated version for production use."
    echo
}
